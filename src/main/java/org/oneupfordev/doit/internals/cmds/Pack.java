...
class Pack implements CallableExpression {
	...
	public Result doIt() {
		...
		if (selectedAction == Action.DEFAULT_SHOW) {
			final StringBuilder packs = new StringBuilder("Packs:\n");
			for (final ExprPackDescriptor packDescr : session.getDictionary().getPackDescriptors()) {
				packs.append("\t").append(packDescr.getName()).append(":\n");
				for (final RootCmdDescriptor rootCmdDescr : packDescr.getDescriptors()) {
